...
Extracted from the current Python/Next.js implementation. Organized by feature area.
...
- **As the first registered user**, I am automatically promoted to admin, so that the system has an initial administrator.
...
- **As an admin**, I want to manually trigger recap generation with a custom date range, so that I can generate on-demand summaries.
...
### LLM Configuration
- **As an admin**, I want to choose between LLM providers (OpenAI, Anthropic, Ollama/local), so that I can use my preferred AI service.
- **As an admin**, I want to select the chat model from available models for the chosen provider, so that I control quality/cost.
- **As an admin**, I want to choose between embedding providers (OpenAI, Sentence Transformers/local), so that I can use cloud or local embeddings.
...
### Chat Behavior
- **As an admin**, I want to configure: number of context chunks (default 15), LLM temperature (default 0.7), and a custom system prompt, so that I can tune response quality.
...
These aren't user stories but are worth keeping in mind for a rebuild:

- **Multi-provider abstraction**: LLM and embedding services use a facade/strategy pattern so providers are swappable at runtime (configured in DB, not env).
- **Background jobs**: Heavy work (ingestion, recap generation, email sending) runs in background workers, not in the request cycle.
- **Vector store**: A vector database is used for semantic search over document embeddings.
- **Token budgeting**: Context for the LLM is assembled with a token budget (~16K) to avoid exceeding model limits.
- **Diff-based ingestion**: Content hashing prevents redundant reprocessing.
- **Conversation summarization**: Long conversations are compressed to keep LLM context manageable.
