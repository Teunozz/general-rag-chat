...
test('anthropic returns hardcoded text models', function () {
...
test('anthropic returns empty for embedding', function () {
    $models = $this->service->fetchModels('anthropic', 'embedding');

    expect($models)->toBeEmpty();
});

test('gemini returns hardcoded models', function () {
...
test('unknown provider returns empty', function () {
    $models = $this->service->fetchModels('unknown-provider');

    expect($models)->toBeEmpty();
});

test('openai with no api key returns empty', function () {
...
test('openai filters text models', function () {
...
test('openai filters embedding models', function () {
    config(['services.openai.api_key' => 'test-key']);
...
test('openai handles api failure', function () {
    config(['services.openai.api_key' => 'test-key']);
...
test('models have id and name keys', function () {
    $models = $this->service->fetchModels('anthropic', 'text');
